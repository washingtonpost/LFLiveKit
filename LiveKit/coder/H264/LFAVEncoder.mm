...
- (void)initForHeight:(int)height andWidth:(int)width bitrate:(int)bitrate {
    ...
    _headerWriter = [VideoEncoder encoderForPath:path Height:height andWidth:width bitrate:(int)self.bitrate];
    _times = [NSMutableArray arrayWithCapacity:10];

    // swap between 3 filenames
    _currentFile = 1;
    _writer = [VideoEncoder encoderForPath:[self makeFilename] Height:height andWidth:width bitrate:(int)self.bitrate];
    ...
    @synchronized(self){
        ...
        if (!_swapping) {
            ...
            if (st.st_size > OUTPUT_FILE_SWITCH_POINT || self.bitrateChanged) {
                ...
                _writer = [VideoEncoder encoderForPath:[self makeFilename] Height:_height andWidth:_width bitrate:(int)self.bitrate];
