...
@property (nonatomic, strong) LiveAudioConfiguration *configuration;
@property (nonatomic, weak) id<AudioEncodingDelegate> aacDeleage;

@end

@implementation LFHardwareAudioEncoder

- (instancetype)initWithAudioStreamConfiguration:(nullable LiveAudioConfiguration *)configuration {
    if (self = [super init]) {
...
#pragma mark -- AudioEncoder
- (void)setDelegate:(id<AudioEncodingDelegate>)delegate {
...
- (void)encodeAudioData:(nullable NSData*)audioData timeStamp:(uint64_t)timeStamp {
    ...
    if(leftLength + audioData.length >= self.configuration.bufferLength){
        ...
        memset(totalBuf, (int)totalSize, sizeof(totalSize));
