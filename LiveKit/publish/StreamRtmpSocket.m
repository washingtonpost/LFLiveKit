...
@interface StreamRTMPSocket ()<StreamingBufferDelegate>
{
    PILI_RTMP *_rtmp;
}
@property (nonatomic, weak) id<StreamSocketDelegate> delegate;
@property (nonatomic, strong) LiveStreamInfo *stream;
@property (nonatomic, strong) StreamingBuffer *buffer;
...
- (nullable instancetype)initWithStream:(nullable LiveStreamInfo *)stream reconnectInterval:(NSInteger)reconnectInterval reconnectCount:(NSInteger)reconnectCount{
    if (!stream) @throw [NSException exceptionWithName:@"StreamRtmpSocket init error" reason:@"stream is nil" userInfo:nil];
    if (self = [super init]) {
        _stream = stream;
...
- (void)setDelegate:(id<StreamSocketDelegate>)delegate {
    _delegate = delegate;
}
        ...
        if (!_self.isSending && _self.buffer.list.count > 0) {
            ...
            if ([frame isKindOfClass:[VideoFrame class]]) {
                if (!_self.sendVideoHead) {
                    ...
                    if(!((VideoFrame*)frame).sps || !((VideoFrame*)frame).pps){
                        _self.isSending = NO;
                        return;
                    }
                    [_self sendVideoHeader:(VideoFrame *)frame];
                } else {
                    [_self sendVideo:(VideoFrame *)frame];
                }
            } else {
                if (!_self.sendAudioHead) {
                    _self.sendAudioHead = YES;
                    if(!((AudioFrame*)frame).audioInfo){
                        _self.isSending = NO;
                        return;
                    }
                    [_self sendAudioHeader:(AudioFrame *)frame];
                } else {
...
- (void)sendAudioHeader:(AudioFrame *)audioFrame {
    ...
    dispatch_async(self.rtmpSendQueue, ^{
        if (self.retryTimes4netWorkBreaken++ < self.reconnectCount && !self.isReconnecting) {
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(socketDidError:errorCode:)]) {
                [self.delegate socketDidError:self errorCode:LiveSocketError_ReConnectTimeOut];
            }
        }
    });
}
...
void RTMPErrorCallback(RTMPError *error, void *userData) {
    StreamRTMPSocket *socket = (__bridge StreamRTMPSocket *)userData;
    if (error->code < 0) {
        [socket reconnect];
    }
}
...
- (StreamingBuffer *)buffer {
    if (!_buffer) {
        _buffer = [[StreamingBuffer alloc] init];
...
- (LiveDebug *)debugInfo {
    if (!_debugInfo) {
        _debugInfo = [[LiveDebug alloc] init];
    }
    return _debugInfo;
}
