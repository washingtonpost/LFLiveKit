...
@implementation LiveVideoConfiguration

#pragma mark -- LifeCycle

+ (instancetype)defaultConfiguration {
    LiveVideoConfiguration *configuration = [LiveVideoConfiguration defaultConfigurationForQuality:LiveVideoQuality_Default];
    return configuration;
}

+ (instancetype)defaultConfigurationForQuality:(LiveVideoQuality)videoQuality {
    LiveVideoConfiguration *configuration = [LiveVideoConfiguration defaultConfigurationForQuality:videoQuality outputImageOrientation:UIInterfaceOrientationPortrait];
    return configuration;
}

+ (instancetype)defaultConfigurationForQuality:(LiveVideoQuality)videoQuality outputImageOrientation:(UIInterfaceOrientation)outputImageOrientation {
    LiveVideoConfiguration *configuration = [LiveVideoConfiguration new];
    switch (videoQuality) {
    case LiveVideoQuality_Low1:{
...
- (void)setSessionPreset:(LiveVideoSessionPreset)sessionPreset{
...
- (BOOL)isEqual:(id)other {
    if (other == self) {
        ...
        LiveVideoConfiguration *object = other;
...
- (id)copyWithZone:(nullable NSZone *)zone {
    LiveVideoConfiguration *other = [self.class defaultConfiguration];
    return other;
}
